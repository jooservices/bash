...
function parse_params() {
        ...
        source_iso="${iso_dir}/${original_iso}"
        autoinstall_iso="focal-autoinstall-live-server-amd64.iso"
        destination_iso="${iso_dir}/${autoinstall_iso}"
        ...
        if [ "${source_iso}" != "${iso_dir}/${original_iso}" ]; then
        ...
        if [ "${use_release_iso}" -eq 1 ]; then
                ...
                original_iso="${download_iso}"
                source_iso="${iso_dir}/${download_iso}"
...
packages=('xorriso' 'sed' 'curl' 'gpg')
for package in "${packages[@]}"
do
        :
        [[ ! -x "$(command -v ${package})" ]] && die "💥 ${package} is not installed. On Ubuntu, install  the 'xorriso' package."        
done
...
cd "$tmpdir"
xorriso -as mkisofs -r -V ${original_iso} -J -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -isohybrid-mbr /usr/lib/ISOLINUX/isohdpfx.bin -boot-info-table -input-charset utf-8 -eltorito-alt-boot -e boot/grub/efi.img -no-emul-boot -isohybrid-gpt-basdat -o "${destination_iso}" . &>/dev/null
cd "$OLDPWD"
